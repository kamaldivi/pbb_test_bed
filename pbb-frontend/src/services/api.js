...
export const apiService = {
  ...
  async getBookPages(bookId) {
    try {
      const response = await api.get(`/api/v1/books/${bookId}/pages/core`);
      console.log('Core Pages API Response Data:', response.data);
      return response.data;
    } catch (error) {
      throw new Error(`Failed to fetch core pages for book ${bookId}: ${error.message}`);
    }
  },
