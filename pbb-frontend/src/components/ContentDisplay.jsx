...
const ContentDisplay = ({ content, loading, error, bookId, pageNumber, pageLabel, onRetry }) => {
  ...
  // Extract page_content from the API response
  // API returns: {content: {book_id, page_number, page_content, ...}, message: null}
  let pageContent = '';
  if (typeof content === 'string') {
    pageContent = content;
  } else if (content && typeof content === 'object') {
    // Handle nested structure: content.content.page_content
    if (content.content && content.content.page_content) {
      pageContent = content.content.page_content;
    } else {
      pageContent = content.page_content || content.content || '';
    }
  }
