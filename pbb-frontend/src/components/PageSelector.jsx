...
const PageSelector = ({
  ...
  if (loading) {
    return (
      <div className="w-full h-full bg-white rounded-lg shadow-md p-3">
        <h3 className="text-sm font-semibold text-gray-800 mb-2">Pages</h3>
      ...
      <div className="max-h-[750px] overflow-y-auto space-y-1 pr-1">
        {sortedPages.map((page) => (
          <button
            ...
            className={`w-full text-left p-3 rounded-xl text-xs font-medium transition-all duration-200 ${
              selectedPage?.page_number === page.page_number
                ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white shadow-lg border border-blue-500 ring-2 ring-blue-300/50'
                : 'bg-white/70 backdrop-blur-sm hover:bg-white text-slate-700 border border-blue-200/50 hover:border-blue-400 hover:shadow-md hover:scale-[1.02]'
            }`}
          >
